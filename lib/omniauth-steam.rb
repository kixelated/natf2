  ...
  module Strategies
    class Steam < OmniAuth::Strategies::OpenID
      ...
      def user_info(response=nil)
        player = user_hash['response']['players'].first
      ...
      def user_hash
        ...
        if @api_key
          unless @user_hash
            ...
            req = Net::HTTP::Get.new("#{uri.path}ISteamUser/GetPlayerSummaries/v0002/?key=#{@api_key}&steamids=#{@openid_response.display_identifier.split("/").last}")
